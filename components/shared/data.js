...
export const PROJECTS = [
  {
    ...
    challenge: "The most interesting part was discovering the need for a database organically. The bot was running giveaways and I realized if it restarted mid-giveaway, the state was gone. That forced me to learn persistent storage, and once I had a database I kept finding more uses for it — tracking infractions, reaction roles, message counts for a leveling system. Another hard problem was content moderation. I started by manually listing every banned word and its variations, which was a losing battle — users could always get more creative. I researched solutions and found the fast-levenshtein package, which scores words by similarity to the original rather than requiring exact matches, making the system much harder to game.",
  ...
  },
  {
    ...
    challenge: "I had never heard of ADBKit or a VM before this project. The core questions were: how do I interact with the game programmatically, and how do I keep it running 24/7? The hardest unsolved problem was moving to an in-game chat approach (how modern bots work) instead of Discord commands — players never have to leave the game. My attempt was a bot watching kingdom chat with OCR, but when a title request came in the watcher would fall behind and miss messages. I tried splitting into two bots — one watching, one assigning — but simultaneous requests still caused drops. I believe the real solution is data interception between the game client and server, which would also explain how some bots expose private player stats, but I never confirmed or implemented this. A secondary hard problem was clicking accuracy: the governor profile menu can appear in 4 different screen positions, so I used Tesseract, Sharp, and PixelMatch to detect its location dynamically on each run.",
  ...
  },
  {
    ...
    challenge: "Cannot be filled in yet — project is in progress.",
    differently: "Cannot be filled in yet — project is in progress.",
